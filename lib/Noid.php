...
class Noid{
	...
	static public function mint($noid, $contact, $pepper = 0): string|NULL
	{
		...
		foreach($values as $key => $value){
			...
			if(Db::$engine->get(Globals::_RR . "/queued") < 0){
				$m = sprintf('error: queued count (%s) going negative on id %s', Db::$engine->get(Globals::_RR . "/queued"), $id);
				Log::addmsg($noid, $m);
				Log::logmsg($noid, $m);
				return NULL;
			}
		...
		while(TRUE){
			...
			# "sequential".
			#
			srand(Db::$engine->get(Globals::_RR . "/oacounter"));
	...
	static public function hold_set($noid, $id): int
	{
		...
		if(Db::$engine->get(Globals::_RR . "/total") != Globals::NOLIMIT   # ie, if total is non-zero
			...
			$m = sprintf('error: hold count (%s) exceeding total possible on id %s', Db::$engine->get(Globals::_RR . "/held"), $id);
			Log::addmsg($noid, $m);
			Log::logmsg($noid, $m);
			return 0;
		}
	...
	static public function hold_release($noid, $id): int
	{
		...
		if(Db::$engine->get(Globals::_RR . "/held") < 0){
			$m = sprintf('error: hold count (%s) going negative on id %s', Db::$engine->get(Globals::_RR . "/held"), $id);
			Log::addmsg($noid, $m);
			Log::logmsg($noid, $m);
			return 0;
		}
