...
if(!class_exists('NoidApp')){
	...
	class NoidApp{
		...
		private function usage($in_error = NULL, $brief = NULL, $topic = NULL)
		{
			...
			if($blurb == 'intro'){
				$msg = 'Usage:
              noid [-f DbDir] [-t DbType] [-v] [-h] Command Arguments';
				if($brief){
					$msg .= '
              noid -h             (for help with a Command summary).';
					$msg .= '
              DbType: ';
					foreach(Globals::DB_TYPES as $key => $item){
						$msg .= "{$key}, ";
					}
					$msg .= ' ...';
...
DbDir defaults to "." if not found from -f/--fdbdir or a NOID environment
variable.
DbType defaults to "bdb" (means Berkeley DB) if not found from -t/--type.
 ...
 * =head1 SYNOPSIS
 *
 * B<noid> [ B<-f> I<DbDir> ] [ B<-t> I<DbType> ] [ B<-vh> ] I<Command> I<Arguments>
 *
 * =head1 DESCRIPTION
 *
